...
class LaneLines:
    ...
    def fit_poly(self, img):
        ...
        Returns:
            out_img (np.array): a RGB image that has lane line drawn on that.
        """
        ...
        # Ensure that self.left_fit and self.right_fit are not None
        if self.left_fit is None:
            self.left_fit = np.zeros(3)  # Initialize with zeros or other default values
        if self.right_fit is None:
            self.right_fit = np.zeros(3)  # Initialize with zeros or other default values

        # Generate x and y values for plotting
        maxy = img.shape[0] - 1
        miny = img.shape[0] // 3

        # Check if self.left_fit is not None before using it
        if self.left_fit is not None:
            maxy = max(maxy, np.max(lefty))
            miny = min(miny, np.min(lefty))

        # Check if self.right_fit is not None before using it
        if self.right_fit is not None:
            maxy = max(maxy, np.max(righty))
            miny = min(miny, np.min(righty))
